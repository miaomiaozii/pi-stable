...
```bash
npm install pi-stable-ai
```

TypeBox exports are re-exported from `pi-stable-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from 'pi-stable-ai';
...
import { openaiProvider } from 'pi-stable-ai/providers/openai';
import { openrouterProvider } from 'pi-stable-ai/providers/openrouter';
import { amazonBedrockProvider } from 'pi-stable-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from 'pi-stable-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from 'pi-stable-ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from 'pi-stable-ai';
...
```typescript
import { Type, type Tool, StringEnum } from 'pi-stable-ai';
...
```typescript
import { validateToolCall, type Tool } from 'pi-stable-ai';
...
```typescript
import { builtinImagesModels } from 'pi-stable-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from 'pi-stable-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from 'pi-stable-ai';
import { openAICompletionsApi } from 'pi-stable-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from 'pi-stable-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from 'pi-stable-ai/api/openai-responses.lazy';
...
```typescript
import { stream } from 'pi-stable-ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `pi-stable-ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from 'pi-stable-ai';
import { anthropicProvider } from 'pi-stable-ai/providers/anthropic';
import { openaiProvider } from 'pi-stable-ai/providers/openai';
import { googleProvider } from 'pi-stable-ai/providers/google';
...
- `pi-stable-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `pi-stable-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `pi-stable-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `pi-stable-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `pi-stable-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from 'pi-stable-ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from 'pi-stable-ai/bedrock-provider';
...
```bash
npx pi-stable-ai login              # interactive provider selection
npx pi-stable-ai login anthropic    # login to specific provider
npx pi-stable-ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `pi-stable-ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from 'pi-stable-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from 'pi-stable-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `pi-stable-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `pi-stable-ai`.
