...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		setExecPath(
			"C:\\Users\\Admin\\Documents\\pnpm-repository\\global\\5\\.pnpm\\pi-stable-@0.67.68\\node_modules\\pi-stable\\dist\\cli.js",
		);

		expect(detectInstallMethod()).toBe("pnpm");
		expect(getUpdateInstruction("pi-stable")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 pi-stable",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getUpdateInstruction("pi-stable")).toBe(
			"Update pi-stable using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("pi-stable");
	...
	test("self-updates exact npm versions without uninstalling the current package", () => {
		...
		const command = getSelfUpdateCommand("pi-stable", undefined, {
			packageName: "pi-stable",
			installSpec: "pi-stable@1.2.3",
		});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("pi-stable", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 pi-stable`,
		);
	});

	test("does not infer Windows npm custom prefixes from package paths", () => {
		const packageDir = "C:\\Users\\Admin\\npm prefix\\node_modules\\pi-stable";
		process.env.PI_PACKAGE_DIR = packageDir;
		setExecPath(`${packageDir}\\dist\\cli.js`);

		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("pi-stable")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 pi-stable",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "pi-stable"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 pi-stable",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "pi-stable";
		const globalPackageDir = join(root, "11e9a", "node_modules", "pi-stable");
