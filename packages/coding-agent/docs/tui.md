...
**Source:** [`pi-stable-tui`](https://github.com/earendil-works/pi-mono/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "pi-stable-tui";
...
```typescript
import { Container, type Focusable, Input } from "pi-stable-tui";
...
Import from `pi-stable-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "pi-stable-tui";
```
...
```typescript
import { matchesKey, Key } from "pi-stable-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "pi-stable-tui";
...
```typescript
import { getMarkdownTheme } from "pi-stable";
import { Markdown } from "pi-stable-tui";
...
For letting users pick from a list of options. Use `SelectList` from `pi-stable-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "pi-stable";
import { DynamicBorder } from "pi-stable";
import { Container, type SelectItem, SelectList, Text } from "pi-stable-tui";
...
```typescript
import { BorderedLoader } from "pi-stable";
...
For toggling multiple settings. Use `SettingsList` from `pi-stable-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "pi-stable";
import { Container, type SettingItem, SettingsList, Text } from "pi-stable-tui";
...
import { matchesKey, truncateToWidth } from "pi-stable-tui";
