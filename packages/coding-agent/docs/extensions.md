...
| Package | Purpose |
|---------|---------|
| `pi-stable` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `pi-stable-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `pi-stable-tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "pi-stable";
...
```typescript
import { isBashToolResult } from "pi-stable";
...
```typescript
import { createLocalBashOperations } from "pi-stable";
...
```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "pi-stable";
...
```typescript
import { SessionManager } from "pi-stable";
...
```typescript
import type { AutocompleteItem } from "pi-stable-tui";
...
```typescript
import { Box, Text } from "pi-stable-tui";
...
Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `pi-stable-ai`. The provider becomes the composition base and `models.json` overrides still apply above it.
...
```typescript
import { withFileMutationQueue } from "pi-stable";
...
**Important:** Use `StringEnum` from `pi-stable-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "pi-stable";
...
```typescript
import { createBashTool } from "pi-stable";
...
```typescript
import { Text, Component } from "pi-stable-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "pi-stable";
import { matchesKey } from "pi-stable-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "pi-stable";
