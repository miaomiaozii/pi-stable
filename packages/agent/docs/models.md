...
`pi-stable-ai/compat` preserves the old global API surface until the coding-agent migration deletes it. New code never imports it.
...
coding-agent (and the interim agent package) switch imports of these symbols from `pi-stable-ai` to `pi-stable-ai/compat` (import-path-only change) and are otherwise untouched until the ModelManager migration.

Extension grace period: the coding-agent extension loader (jiti aliases + Bun `virtualModules`) resolves the `pi-stable-ai` ROOT specifier to the compat entrypoint. Existing user extensions using the old global API (`complete`, `getModel`, `registerApiProvider`, ...) keep working at runtime without changes; they break only when compat is removed at the ModelManager migration, with a migration guide in the changelog. Typechecking is the nudge: editors resolve the root to the slim core types, so extension sources that typecheck must import old globals from `/compat` — which is what the repo example extensions demonstrate.
...
1. Main `pi-stable-ai` import is core-only.
...
- coding-agent still uses legacy `AuthStorage` + `ModelRegistry` and imports old global pi-ai APIs through `pi-stable-ai/compat`.
...
- [x] Switch old-global imports to `pi-stable-ai/compat` (landed with Phase 5; compat is a superset so the switch was path-only). Extension loader resolves the pi-ai root to compat as the runtime grace period.
