# pi-stable-agent-core

Stateful agent with tool execution and event streaming. Built on `pi-stable-ai`.

## Installation

```bash
npm install pi-stable-agent-core
```

### SQLite session backends

The SQLite session backend and the `node:sqlite` adapter live in a separate package, `pi-stable-session-backend-sqlite-node`, so the core package does not pull in runtime builtins or native SQLite dependencies by default. The backend accepts a runtime-specific SQLite factory, allowing other session backends to ship as their own packages in the future.

## Quick Start

```typescript
import { Agent } from "pi-stable-agent-core";
...
```typescript
declare module "pi-stable-agent-core" {
...
```typescript
import { Agent, streamProxy } from "pi-stable-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "pi-stable-agent-core";
