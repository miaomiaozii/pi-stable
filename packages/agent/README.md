...
The SQLite session backend and the `node:sqlite` adapter live in a separate package, `pi-stable-storage-sqlite-node`, so the core package does not pull in runtime builtins or native SQLite dependencies by default. The backend accepts a runtime-specific SQLite factory, allowing other storage backends to ship as their own packages in the future.
