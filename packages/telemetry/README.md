...
```bash
npm install pi-stable-telemetry
```
...
```typescript
import { NOOP_TELEMETRY_CONTEXT } from 'pi-stable-telemetry';
...
```typescript
import { InMemoryTelemetryContext } from 'pi-stable-telemetry';
...
`pi-stable-telemetry/testing` exports a runner-independent conformance suite modeled as grouped cases. A fixture supplies a fresh context and converts its backend's finished spans into normalized `RecordedTelemetrySpan` snapshots:

```typescript
import {
  createTelemetryAdapterConformance,
  type TelemetryAdapterFixture,
} from 'pi-stable-telemetry/testing';
...
```typescript
import { AGENT_TELEMETRY_SCHEMAS } from 'pi-stable-agent-core';
...
- `pi-stable-telemetry` owns the vendor-neutral contract, no-op and in-memory reference contexts, schema utilities, and adapter conformance suite;
- `pi-stable-ai` accepts and propagates `telemetryContext` in provider request options but owns no telemetry schema;
- `pi-stable-agent-core` owns and exports the pi AI-request and harness schemas, their combined readonly schema tuple, and typed span helpers.
