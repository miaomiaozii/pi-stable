...
export async function persistEvalArtifactReferences(
	...
	for (const artifact of artifacts) {
		if (
			(artifact.type !== "pi-stable-evals:session" && artifact.type !== "pi-stable-evals:source") ||
			artifact.runId !== runId
		) {
			continue;
		}
		const category = artifact.type === "pi-stable-evals:session" ? "sessions" : "sources";
