...
```ts
import { PiClient, type ByteTransportFactory } from "pi-stable-client";
...
```ts
import { PiClient } from "pi-stable-client";
import { createUnixTransportFactory } from "pi-stable-client/unix";
...
The `pi-stable-client` root remains transport- and runtime-neutral. Importing the Node-compatible transport requires the explicit `pi-stable-client/unix` subpath.
